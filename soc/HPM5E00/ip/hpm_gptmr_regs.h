 ...
 * BURST_MODE (RW)
 *
 * set to enable burst mode, timer will reload configured times(burst_cfg), then stop. user need clear CEN and set it to start timer agian.
 * NOTE: do not set burst_mode and opmode at same time
 */
#define GPTMR_CHANNEL_CR_BURST_MODE_MASK (0x80000UL)
#define GPTMR_CHANNEL_CR_BURST_MODE_SHIFT (19U)
#define GPTMR_CHANNEL_CR_BURST_MODE_SET(x) (((uint32_t)(x) << GPTMR_CHANNEL_CR_BURST_MODE_SHIFT) & GPTMR_CHANNEL_CR_BURST_MODE_MASK)
#define GPTMR_CHANNEL_CR_BURST_MODE_GET(x) (((uint32_t)(x) & GPTMR_CHANNEL_CR_BURST_MODE_MASK) >> GPTMR_CHANNEL_CR_BURST_MODE_SHIFT)

/*
 * CNT_MODE (RW)
 *
 * 0: internal counting mode, timer increase each gptmr clock cycle.
 * 1: external counting mode, timer increase at each input signal posedge, reload/compare feature can still work but change at input signal posedge.
 */
#define GPTMR_CHANNEL_CR_CNT_MODE_MASK (0x40000UL)
#define GPTMR_CHANNEL_CR_CNT_MODE_SHIFT (18U)
#define GPTMR_CHANNEL_CR_CNT_MODE_SET(x) (((uint32_t)(x) << GPTMR_CHANNEL_CR_CNT_MODE_SHIFT) & GPTMR_CHANNEL_CR_CNT_MODE_MASK)
#define GPTMR_CHANNEL_CR_CNT_MODE_GET(x) (((uint32_t)(x) & GPTMR_CHANNEL_CR_CNT_MODE_MASK) >> GPTMR_CHANNEL_CR_CNT_MODE_SHIFT)
 ...
 *
 */
#define GPTMR_CHANNEL_BURST_CFG_BURST_CFG_MASK (0xFFFFU)
#define GPTMR_CHANNEL_BURST_CFG_BURST_CFG_SHIFT (0U)
#define GPTMR_CHANNEL_BURST_CFG_BURST_CFG_SET(x) (((uint32_t)(x) << GPTMR_CHANNEL_BURST_CFG_BURST_CFG_SHIFT) & GPTMR_CHANNEL_BURST_CFG_BURST_CFG_MASK)
#define GPTMR_CHANNEL_BURST_CFG_BURST_CFG_GET(x) (((uint32_t)(x) & GPTMR_CHANNEL_BURST_CFG_BURST_CFG_MASK) >> GPTMR_CHANNEL_BURST_CFG_BURST_CFG_SHIFT)
 ...
 *
 */
#define GPTMR_CHANNEL_BURST_COUNT_BURST_COUNT_MASK (0xFFFFU)
#define GPTMR_CHANNEL_BURST_COUNT_BURST_COUNT_SHIFT (0U)
#define GPTMR_CHANNEL_BURST_COUNT_BURST_COUNT_GET(x) (((uint32_t)(x) & GPTMR_CHANNEL_BURST_COUNT_BURST_COUNT_MASK) >> GPTMR_CHANNEL_BURST_COUNT_BURST_COUNT_SHIFT)
 ...
 * QEI_TYPE23 (RW)
 *
 * define capture_in process for channel 2/3
 */
#define GPTMR_GCR_QEI_TYPE23_MASK (0xC0000UL)
#define GPTMR_GCR_QEI_TYPE23_SHIFT (18U)
#define GPTMR_GCR_QEI_TYPE23_SET(x) (((uint32_t)(x) << GPTMR_GCR_QEI_TYPE23_SHIFT) & GPTMR_GCR_QEI_TYPE23_MASK)
#define GPTMR_GCR_QEI_TYPE23_GET(x) (((uint32_t)(x) & GPTMR_GCR_QEI_TYPE23_MASK) >> GPTMR_GCR_QEI_TYPE23_SHIFT)

/*
 * QEI_TYPE01 (RW)
 *
 * define capture_in process for channel 0/1, for qei mode only:
 * 00: ud mode. nothing changed to capture_in.
 * 01: ab mode, the capture_in will be asserted at posedge when another channel capture_in is high;
 * 10: pd mode, capture_in[0] as signal,  capture_in[1] as direction.
 * if capture_in[1] is high, then capture_in[0] is connected to channel0, 0 is connected to cahnnel1;
 * if capture_in[1] is low, then capture_in[0] is connected to channel1, 0 is connected to cahnnel0;
 * 11: reserved
 */
#define GPTMR_GCR_QEI_TYPE01_MASK (0x30000UL)
#define GPTMR_GCR_QEI_TYPE01_SHIFT (16U)
#define GPTMR_GCR_QEI_TYPE01_SET(x) (((uint32_t)(x) << GPTMR_GCR_QEI_TYPE01_SHIFT) & GPTMR_GCR_QEI_TYPE01_MASK)
#define GPTMR_GCR_QEI_TYPE01_GET(x) (((uint32_t)(x) & GPTMR_GCR_QEI_TYPE01_MASK) >> GPTMR_GCR_QEI_TYPE01_SHIFT)
