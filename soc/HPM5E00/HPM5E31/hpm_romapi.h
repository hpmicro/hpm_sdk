 ...
 * @retval status_invalid  Invalid parameters were detected
 */
...
/**
 * @brief Power on the SIP FLASH
 * @param [in] base  XPI base address
 */
static inline void rom_xpi_nor_power_on_sip_flash(XPI_Type *base)
{
    ROM_API_TABLE_ROOT->xpi_nor_driver_if->sip_flash_power_on(base);
}
/**
 * @brief Power off the SIP FLASH
 * @param [in] base XPI base address
 */
static inline void rom_xpi_nor_power_off_sip_flash(XPI_Type *base)
{
    ROM_API_TABLE_ROOT->xpi_nor_driver_if->sip_flash_power_off(base);
}

/**
 * @brief Enable Hybrid XPI mode
 * @param [in] base XPI base address
 */
static inline void rom_xpi_nor_enable_hybrid_mode(XPI_Type *base)
{
    ROM_API_TABLE_ROOT->xpi_nor_driver_if->enable_hybrid_xpi(base);
}

/**
 * @brief Disable Hybrid XPI mode
 * @param [in] base XPI base address
 */
static inline void rom_xpi_nor_disable_hybrid_mode(XPI_Type *base)
{
    ROM_API_TABLE_ROOT->xpi_nor_driver_if->disable_hybrid_xpi(base);
}

/**
 * @brief Post Erase Sector Nonblocking API (required only if Hybrid XPI mode is used)
 * @param [in] base XPI base address
 * @param [in] chn XPI channel
 * @param [in] nor_cfg XPI NOR configuration context
 * @param [in] addr Sector address
 *
 * @return Operation status
 */
ATTR_RAMFUNC static inline hpm_stat_t rom_xpi_nor_post_erase_sector_nonblocking(XPI_Type *base,
                                                                                xpi_xfer_channel_t chn,
                                                                                xpi_nor_config_t *nor_cfg,
                                                                                uint32_t addr)
{
    return ROM_API_TABLE_ROOT->xpi_nor_driver_if->post_erase_sector_nonblocking(base, chn, nor_cfg, addr);
}

/**
 * @brief Post Erase Block Nonblocking API (required only if Hybrid XPI mode is used)
 * @param [in] base XPI base address
 * @param [in] chn XPI channel
 * @param [in] nor_cfg XPI NOR configuration context
 * @param [in] addr Sector address
 ...
 * @return Operation status
 */
ATTR_RAMFUNC static inline hpm_stat_t rom_xpi_nor_post_erase_block_nonblocking(XPI_Type *base,
                                                                               xpi_xfer_channel_t chn,
                                                                               xpi_nor_config_t *nor_cfg,
                                                                               uint32_t addr)
{
    return ROM_API_TABLE_ROOT->xpi_nor_driver_if->post_erase_block_nonblocking(base, chn, nor_cfg, addr);
}

/**
 * @brief Post Erase Chip Nonblocking API (required only if Hybrid XPI mode is used)
 * @param [in] base XPI base address
 * @param [in] chn XPI channel
 * @param [in] nor_cfg XPI NOR configuration context
 *
 * @return Operation status
 */
ATTR_RAMFUNC static inline hpm_stat_t rom_xpi_nor_post_erase_chip_nonblocking(XPI_Type *base,
                                                                              xpi_xfer_channel_t chn,
                                                                              xpi_nor_config_t *nor_cfg)
{
    return ROM_API_TABLE_ROOT->xpi_nor_driver_if->post_erase_chip_nonblocking(base, chn, nor_cfg);
}

/**
 * @brief Post Page Program Nonblocking API (required only if Hybrid XPI mode is used)
 * @param [in] base XPI base address
 * @param [in] chn XPI channel
 * @param [in] nor_cfg XPI NOR configuration context
 * @param [in] src Data source pointer
 * @param [in] dst_addr Destination address
 * @param [in] length Size of bytes to be written
 */
ATTR_RAMFUNC static inline hpm_stat_t rom_xpi_nor_post_program_nonblocking(XPI_Type *base,
                                                                           xpi_xfer_channel_t chn,
                                                                           xpi_nor_config_t *nor_cfg,
                                                                           const uint32_t *src,
                                                                           uint32_t dst_addr,
                                                                           uint32_t length)
{
    return ROM_API_TABLE_ROOT->xpi_nor_driver_if->post_page_program_nonblocking(base,
                                                                                chn,
                                                                                nor_cfg,
                                                                                src,
                                                                                dst_addr,
