...
static hpm_stat_t _sysctl_cpu_set_gpr(SYSCTL_Type *ptr, uint8_t cpu, uint8_t start, uint8_t count, uint32_t *data)
{
    uint8_t i, size = ARRAY_SIZE(ptr->CPU[cpu].GPR);
    if (!sysctl_valid_cpu_index(cpu) || (data == NULL) || !count || start > size || count > size ||
        (start + count) > size) {
...
{
    uint32_t index;
    if ((node < clock_node_adc_i2s_start) || (source >= clock_source_adc_i2s_clk_end)) {
        return status_invalid_argument;
    }
    ...
    case clock_node_i2s0:
        ...
        ptr->I2SCLK[index] = (ptr->I2SCLK[index] & ~SYSCTL_I2SCLK_MUX_MASK) | SYSCTL_I2SCLK_MUX_SET(source);
        break;
    default:
        return status_invalid_argument;
    }
