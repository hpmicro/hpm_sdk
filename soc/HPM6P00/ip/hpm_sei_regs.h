...
typedef struct {
    struct {
        ...
        struct {
            ...
            __RW uint32_t IN_DIV;              /* 0x54: Trigger input divider */
            __R  uint8_t  RESERVED0[8];        /* 0x58 - 0x5F: Reserved */
            __R  uint32_t PRD_STS;             /* 0x60: Period trigger status */
            __R  uint32_t PRD_CNT;             /* 0x64: Period trigger counter */
            __R  uint32_t DIV_STS;             /* 0x68: Trigger input divider status */
            __R  uint8_t  RESERVED1[20];       /* 0x6C - 0x7F: Reserved */
        } TRG;
        struct {
 ...
 * REWIND_EN (RW)
 *
 * enable rewind cmd register by LATCH
 */
 ...
 * REWIND_SEL (RW)
 *
 * select one LATCH to rewind CMD register
 * 0:LATCH[0]
 * 1:LATCH[1]
 * 2:LATCH[2]
 * 3:LATCH[3]
...
/* Bitfield definition for register of struct array CTRL: IN_DIV */
/*
 * IN1_DIV (RW)
 *
 * The divider of trigger in 1.
 * 0x0: disable divider
 * 0x1: divider by 2
 * 0x2: divider by 3
 * ...
...
/* Bitfield definition for register of struct array CTRL: DIV_STS */
/*
 * IN1_CNT (RO)
 *
 * The divider counter for trigger in 1, trigger valid when counter is 0.
 */
 ...
 * IN0_CNT (RO)
 *
 * The divider counter for trigger in 0, trigger valid when counter is 0.
 */
