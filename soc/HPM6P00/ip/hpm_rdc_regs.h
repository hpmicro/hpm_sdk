...
typedef struct {
    ...
    __RW uint32_t IIR_B;                       /* 0x14: IIR parameter for b branch */
    __RW uint32_t IIR_A;                       /* 0x18: IIR parameter for a branch */
    __R  uint8_t  RESERVED0[24];               /* 0x1C - 0x33: Reserved */
    ...
    __R  uint8_t  RESERVED1[16];               /* 0x5C - 0x6B: Reserved */
    __RW uint32_t MAX_MIN_POS;                 /* 0x6C: max min data position of channel */
 ...
 * ACC_FAST (RW)
 *
 * every adc value can be  as one accumulate value
 */
...
/* Bitfield definition for register: IIR_A */
/*
 * IIR_A2 (RW)
 *
 * IIR parameter a2 for a branch
 */
...
/* Bitfield definition for register: MAX_MIN_POS */
/*
 * MAX_MIN_POS (RW)
 *
 * max min value position
 * 0: max min value at adc input
 * 1: max min value at IIR output
 */
 ...
 * THRS4ACC (RW)
 *
 * enable thrs data for accumulate
 */
