...
hpm_stat_t board_init_enet_rmii_reference_clock(ENET_Type *ptr, bool internal)
{
    ...
    if (ptr == HPM_ENET0) {
        if (BOARD_RUNNING_CORE & 0x1) {
            init_enet0_rmii_clock_group1();
        } else {
            init_enet0_rmii_clock();
        }
        if (internal) {
            ...
            if (pllctl_init_int_pll_with_freq(HPM_PLLCTL, PLLCTL_PLL_PLL2, 1000000000UL) == status_success) {
                ...
                init_enet_rmii_ref_pll2_postdiv();
                /* set eth clock frequency at 50MHz for enet0 */
                init_enet0_rmii_ref_clock();
            } else {
                return status_fail;
            }
        }
        ...
        if (BOARD_RUNNING_CORE & 0x1) {
            init_enet1_rmii_clock_group1();
        } else {
            init_enet1_rmii_clock();
        }
        if (internal) {
            /* set pll output frequency at 1GHz */
            if (pllctl_init_int_pll_with_freq(HPM_PLLCTL, PLLCTL_PLL_PLL2, 1000000000UL) == status_success) {
                /* set pll2_clk1 output frequency at 250MHz from PLL2 divided by 4 */
                init_enet_rmii_ref_pll2_postdiv();
                /* set eth clock frequency at 50MHz for enet1 */
                init_enet1_rmii_ref_clock();
            } else {
                return status_fail;
            }
        }
...
hpm_stat_t board_init_enet_rgmii_clock_delay(ENET_Type *ptr)
{
    if (ptr == HPM_ENET0) {
        if (BOARD_RUNNING_CORE & 0x1) {
            init_enet0_rgmii_clock_group1();
        } else {
            init_enet0_rgmii_clock();
        }
...
static hpm_stat_t board_get_enet_phy_config(ENET_Type *ptr, hpm_enet_phy_port_config_t *phy_config)
{
    ...
    if (ptr == HPM_ENET0) {
        ...
        phy_config->type = hpm_enet_phy_rtl8201;
...
#if defined(CONFIG_ENET_PHY) && CONFIG_ENET_PHY
static ENET_Type *board_enet_phy_port_to_base(uint8_t idx)
{
#if defined(HPM_ENET_RGMII) && HPM_ENET_RGMII && defined(HPM_ENET_RMII) && HPM_ENET_RMII
    return (idx == 0U) ? HPM_ENET0 : HPM_ENET1;
#elif defined(HPM_ENET_RMII) && HPM_ENET_RMII
    (void) idx;
    return BOARD_ENET_RMII;
#elif defined(HPM_ENET_RGMII) && HPM_ENET_RGMII
    (void) idx;
...
void board_get_enet_phy_status(uint8_t idx, void *status)
{
    ...
    if ((idx >= BOARD_ENET_COUNT) || (status == NULL)) {
        return;
    }

    stat = board_get_enet_phy_config(board_enet_phy_port_to_base(idx), &phy_config);
    if (stat != status_success) {
        return;
    }
