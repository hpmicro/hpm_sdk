...
void board_init_clock(void)
{
    ...
    /* Configure PLL1 clock frequency and Post Divider */
...
uint32_t board_init_adc_clock(void *ptr, bool clk_src_bus)
{
    ...
    if (ptr == (void *)HPM_ADC0) {
        if (clk_src_bus) {
            /* Configure the ADC clock from AHB (@162MHz by default) */
            init_adc0_bus_clock();
        } else {
            /* Configure the ADC clock from pll0_clk1 via ana divider (/2, @167MHz by default) */
...
uint32_t board_sd_configure_clock(SDXC_Type *ptr, uint32_t freq, bool need_inverse)
{
    ...
    do {
        ...
        init_sdxc0_clock_source();
        sdxc_enable_freq_selection(ptr);

        hpm_stat_t status = clock_wait_source_stable(clock_sdxc0);
        ...
        actual_freq = clock_get_frequency(clock_sdxc0) / sdxc_get_clock_divider(ptr);
    } while (false);
...
#if defined(HPM_ENET_RMII) && HPM_ENET_RMII
...
{
    if (idle_high == true) {
        init_trgm_gptmr2_cap2_invert_pins();
    } else {
        init_gptmr2_channel0_pin_as_capture();
    }
}
