...
#if defined(FMSTR_PLATFORM_HPM)
    #if FMSTR_PLATFORM_HPM
    #include "freemaster_hpm.h"
    #define FMSTR_PLATFORM HPM 
    #else
    #undef FMSTR_PLATFORM_HPM
    #endif
#endif
...
#define FMSTR_COUNT_INTERFACES \
    ( ((FMSTR_DISABLE)?1:0) + ((FMSTR_USE_SCI)?1:0) + ((FMSTR_USE_ESCI)?1:0) + ((FMSTR_USE_LPUART)?1:0) + \
      ...
      ((FMSTR_USE_MQX_IO)?1:0) + ((FMSTR_USE_USB_CDC)?1:0) + ((FMSTR_USE_MBED)?1:0) + ((FMSTR_USE_UART)?1:0))
...
#if (FMSTR_USE_SCI) || (FMSTR_USE_ESCI) || (FMSTR_USE_LPUART) || (FMSTR_USE_JTAG) || (FMSTR_USE_USB_CDC) || (FMSTR_USE_MQX_IO) || (FMSTR_USE_MBED) || (FMSTR_USE_UART)
