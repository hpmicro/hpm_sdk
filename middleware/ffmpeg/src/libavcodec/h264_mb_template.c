...
__attribute__((section(".text.mb_tem"))) static av_noinline void FUNC(hl_decode_mb)(const H264Context *h, H264SliceContext *sl)
{
...
__attribute__((section(".fast"))) static av_noinline void FUNC(hl_decode_mb_444)(const H264Context *h, H264SliceContext *sl)
{
    const int mb_x    = sl->mb_x;
    const int mb_y    = sl->mb_y;
    const int mb_xy   = sl->mb_xy;
    ...
    if (flag == 1) {
        flag = 0;
        printf("transform_bypass is %d\nplane_count is %d\n", transform_bypass, plane_count);
    }
