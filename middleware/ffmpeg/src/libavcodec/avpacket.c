...
AVPacket *av_packet_alloc(void)
{
    ...
    printf("%s %d, alloc %d bytes addr : 0x%x\r\n", __FUNCTION__, __LINE__, sizeof(AVPacket), (void*)pkt);
    if (!pkt)
        return pkt;
...
int av_packet_ref(AVPacket *dst, const AVPacket *src)
{
    ...
    if (!src->buf) {
        ...
        if (ret < 0)
            goto fail;
        printf("%s %d, av_packet_ref %d bytes \r\n", __FUNCTION__, __LINE__, src->size);
