...
ATTR_RAMFUNC static void h264_er_decode_mb(void *opaque, int ref, int mv_dir, int mv_type,
                              int (*mv)[2][4][2],
...
ATTR_RAMFUNC void ff_h264_draw_horiz_band(const H264Context *h, H264SliceContext *sl,
                             int y, int height)
{
 ...
 */
ATTR_RAMFUNC int ff_h264_check_intra4x4_pred_mode(const H264Context *h, H264SliceContext *sl)
{
 ...
 * change the dc mode so it only uses the available blocks.
 */
ATTR_RAMFUNC int ff_h264_check_intra_pred_mode(const H264Context *h, H264SliceContext *sl,
...
ATTR_RAMFUNC const uint8_t *ff_h264_decode_nal(H264Context *h, H264SliceContext *sl,
                                  const uint8_t *src,
 ...
 */
ATTR_RAMFUNC static int decode_rbsp_trailing(H264Context *h, const uint8_t *src)
{
    int v = *src;
    int r;
...
ATTR_RAMFUNC void ff_h264_free_tables(H264Context *h)
{
    int i;
...
ATTR_RAMFUNC int ff_h264_alloc_tables(H264Context *h)
{
 ...
 */
ATTR_RAMFUNC int ff_h264_slice_context_init(H264Context *h, H264SliceContext *sl)
{
    ERContext *er = &sl->er;
...
ATTR_RAMFUNC static int decode_nal_units(H264Context *h, const uint8_t *buf, int buf_size,
                            int parse_extradata);

ATTR_RAMFUNC int ff_h264_decode_extradata(H264Context *h, const uint8_t *buf, int size)
{
    AVCodecContext *avctx = h->avctx;
    int ret;
...
ATTR_RAMFUNC static int h264_init_context(AVCodecContext *avctx, H264Context *h)
{
    int i;
...
ATTR_RAMFUNC av_cold int ff_h264_decode_init(AVCodecContext *avctx)
{
    H264Context *h = avctx->priv_data;
    int ret;
...
#if HAVE_THREADS
ATTR_RAMFUNC static int decode_init_thread_copy(AVCodecContext *avctx)
{
    H264Context *h = avctx->priv_data;
    int ret;
 ...
 */
ATTR_RAMFUNC static void decode_postinit(H264Context *h, int setup_finished)
{
...
ATTR_RAMFUNC int ff_pred_weight_table(H264Context *h, H264SliceContext *sl)
{
    int list, i;
 ...
 */
ATTR_RAMFUNC static void idr(H264Context *h)
{
    int i;
...
ATTR_RAMFUNC void ff_h264_flush_change(H264Context *h)
{
    int i, j;
...
/* forget old pics after a seek */
ATTR_RAMFUNC static void flush_dpb(AVCodecContext *avctx)
{
    H264Context *h = avctx->priv_data;
    int i;
...
ATTR_RAMFUNC int ff_init_poc(H264Context *h, int pic_field_poc[2], int *pic_poc)
{
 ...
 */
ATTR_RAMFUNC int ff_h264_get_profile(SPS *sps)
{
...
ATTR_RAMFUNC int ff_set_ref_count(H264Context *h, H264SliceContext *sl)
{
...
ATTR_RAMFUNC static int get_bit_length(H264Context *h, const uint8_t *buf,
...
ATTR_RAMFUNC static int get_last_needed_nal(H264Context *h, const uint8_t *buf, int buf_size)
{
...
again:
            ...
            if (err < 0 || err == SLICE_SKIPED) {
                if (err < 0)
                    av_log(h->avctx, AV_LOG_ERROR, "decode_slice_header error %d\n", err);
 ...
 */
ATTR_RAMFUNC static int get_consumed_bytes(int pos, int buf_size)
{
    if (pos == 0)
...
ATTR_RAMFUNC static int output_frame(H264Context *h, AVFrame *dst, H264Picture *srcp)
{
    AVFrame *src = srcp->f;
    ...
    for (i = 0; i < desc->nb_components; i++) {
        ...
        uint8_t *tmp = dst->data[i];
        dst->data[i] += off;
        // printf("output frame %d : 0x%x ori: 0x%x\n", i, dst->data[i], tmp);
    }
    return 0;
}

ATTR_RAMFUNC static int is_extra(const uint8_t *buf, int buf_size)
{
    int cnt= buf[5]&0x1f;
...
ATTR_RAMFUNC static int h264_decode_frame(AVCodecContext *avctx, void *data,
...
ATTR_RAMFUNC av_cold void ff_h264_free_context(H264Context *h)
{
    int i;
