                   ...
                   enum AVPixelFormat pix_fmt, int width, int height)
{
    ...
    if (desc->flags & AV_PIX_FMT_FLAG_PAL ||
        ...
        //                (uint32_t)core_local_mem_to_sys_address(HPM_CORE0, (uint32_t)dst_data[i]),
        //                dst_linesizes[i],
        //                (uint32_t)core_local_mem_to_sys_address(HPM_CORE0, (uint32_t)src_data[i]),
        //                src_linesizes[i],
        //                0, 0, src_linesizes[i], planes_nb,
        //                src_linesizes[i], planes_nb,
        //                0xFF, display_pixel_format_yuv422, true, &status);
    }
}
