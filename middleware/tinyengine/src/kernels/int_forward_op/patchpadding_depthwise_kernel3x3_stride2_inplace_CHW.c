...
#include "hpm_math.h" //TODO: remove this in the future for self-contained
		...
		q7_t *cols_8b_iterptr, const uint16_t column_x, int channel_offset)
{
	...
	for (i = 0; i < output_y; i++) {
		for (j = 0; j < output_x / 2; j++) {
            ...
            /* requantize */
            sum0 = hpm_nn_requantize(sum0 + biasR[0], *multiplier, *shift);
            sum0 += output_offset;
            sum0 = MAX(sum0, activation_min);
            sum0 = MIN(sum0, activation_max);
            output[(i * output_x + j * 2) * channel_offset] = sum0;

            sum1 = hpm_nn_requantize(sum1 + biasR[0], *multiplier, *shift);
            sum1 += output_offset;
        ...
        if (output_x & 1) {
            ...
            sum = hpm_nn_requantize(sum + biasR[0], *multiplier, *shift);
			sum += output_offset;
