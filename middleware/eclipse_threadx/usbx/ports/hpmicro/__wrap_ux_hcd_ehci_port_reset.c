...
#elif defined(AZURE_USBX_HOST_HPMICRO)
#include "hpm_usb_drv.h"

/* The HCOR is 0x140 offset of USB_BASE, phy status is 0x224 offset of USB_BASE,
   So phy status is [(0x224 - 0x140) / 4 =  0x39] offset of 32bit pointer 
*/
#define HPM_PHY_STATUS                     (hcd_ehci -> ux_hcd_ehci_hcor + 0x39)
...
#if defined(AZURE_USBX_HOST_HPMICRO)
    ULONG phy_statue = (_ux_hcd_ehci_register_read(hcd_ehci, HPM_PHY_STATUS) & USB_PHY_STATUS_LINE_STATE_MASK) >> USB_PHY_STATUS_LINE_STATE_SHIFT;
    if (phy_statue == usb_line_state2)
    {
        ehci_register_port_status |= USB_PORTSC1_STS_MASK;
    }
    else
    {
        ehci_register_port_status &= ~USB_PORTSC1_STS_MASK;
