...
static const CHAR *nx_hpmicro_interface_name[2];

#ifdef HPM_ENET0
ATTR_PLACE_AT_NONCACHEABLE_BSS NX_DRIVER_INFORMATION nx_driver_information_eth0;
NX_DRIVER_INFORMATION *enet0_drv_info_p;
ATTR_PLACE_AT_NONCACHEABLE_BSS_WITH_ALIGNMENT(ENET_SOC_DESC_ADDR_ALIGNMENT)
__RW enet_rx_desc_t dma_rx_desc_tab_eth0[ENET_RX_BUFF_COUNT]; /* Ethernet Rx DMA Descriptor */

ATTR_PLACE_AT_NONCACHEABLE_BSS_WITH_ALIGNMENT(ENET_SOC_DESC_ADDR_ALIGNMENT)
__RW enet_tx_desc_t dma_tx_desc_tab_eth0[ENET_TX_BUFF_COUNT]; /* Ethernet Tx DMA Descriptor */

ATTR_PLACE_AT_NONCACHEABLE_BSS_WITH_ALIGNMENT(ENET_SOC_BUFF_ADDR_ALIGNMENT)
__RW UCHAR rx_buff_eth0[ENET_RX_BUFF_COUNT][ENET_RX_BUFF_SIZE]; /* Ethernet Receive Buffer */

#if (NETX_TX_DATA_COPY_ALGORITHM == NETX_DATA_COPY_CPU)
ATTR_PLACE_AT_NONCACHEABLE_BSS_WITH_ALIGNMENT(ENET_SOC_BUFF_ADDR_ALIGNMENT)
__RW UCHAR tx_buff_eth0[ENET_TX_BUFF_COUNT][ENET_TX_BUFF_SIZE]; /* Ethernet Transmit Buffer */
#endif
UCHAR mac_eth0[ENET_MAC_SIZE];
#endif
#ifdef HPM_ENET1
ATTR_PLACE_AT_NONCACHEABLE_BSS NX_DRIVER_INFORMATION nx_driver_information_eth1;
NX_DRIVER_INFORMATION *enet1_drv_info_p;
ATTR_PLACE_AT_NONCACHEABLE_BSS_WITH_ALIGNMENT(ENET_SOC_DESC_ADDR_ALIGNMENT)
__RW enet_rx_desc_t dma_rx_desc_tab_eth1[ENET_RX_BUFF_COUNT]; /* Ethernet Rx DMA Descriptor */

ATTR_PLACE_AT_NONCACHEABLE_BSS_WITH_ALIGNMENT(ENET_SOC_DESC_ADDR_ALIGNMENT)
__RW enet_tx_desc_t dma_tx_desc_tab_eth1[ENET_TX_BUFF_COUNT]; /* Ethernet Tx DMA Descriptor */

ATTR_PLACE_AT_NONCACHEABLE_BSS_WITH_ALIGNMENT(ENET_SOC_BUFF_ADDR_ALIGNMENT)
__RW UCHAR rx_buff_eth1[ENET_RX_BUFF_COUNT][ENET_RX_BUFF_SIZE]; /* Ethernet Receive Buffer */

#if (NETX_TX_DATA_COPY_ALGORITHM == NETX_DATA_COPY_CPU)
ATTR_PLACE_AT_NONCACHEABLE_BSS_WITH_ALIGNMENT(ENET_SOC_BUFF_ADDR_ALIGNMENT)
__RW UCHAR tx_buff_eth1[ENET_TX_BUFF_COUNT][ENET_TX_BUFF_SIZE]; /* Ethernet Transmit Buffer */
#endif
UCHAR mac_eth1[ENET_MAC_SIZE];
#endif
...
{
    NX_DRIVER_INFORMATION *p;
    if (enet == HPM_ENET0) {
        p = &nx_driver_information_eth0;
        enet0_drv_info_p = p;
    } else if (enet == HPM_ENET1) {
        p = &nx_driver_information_eth1;
        enet1_drv_info_p = p;
    } else {
        assert(0);
    }
    p->is_rgmii = is_rgmii;
    p->base = enet;
    p->nx_driver_information_ip_ptr = ip;
    p->itf = itf;

}

ATTR_RAMFUNC NX_DRIVER_INFORMATION *get_info_struct_from_ip_driver(NX_IP *ip)
{
    assert(enet0_drv_info_p != NULL);
    assert(enet1_drv_info_p != NULL);
    if (enet0_drv_info_p->nx_driver_information_ip_ptr == ip) {
        return enet0_drv_info_p;
    }
    if (enet1_drv_info_p->nx_driver_information_ip_ptr == ip) {
        return enet1_drv_info_p;
    }
    assert(0);
    return NULL;
}
...
static VOID _nx_driver_hardware_packet_transmitted(NX_IP_DRIVER *driver_req_ptr);
static VOID _nx_driver_hardware_packet_receive(NX_IP_DRIVER *driver_req_ptr);
VOID isr_enet(ENET_Type *ptr, volatile NX_DRIVER_INFORMATION *p)
{
    UINT status;
    UINT rxgbfrmis;
    UINT intr_status;
    ...
    if (ENET_DMA_STATUS_RI_GET(status)) {
        ...
        p->nx_driver_information_deferred_events |= NX_DRIVER_DEFERRED_PACKET_RECEIVED;
        _nx_ip_driver_deferred_processing(p->nx_driver_information_ip_ptr);
#else
    ...
    if (ENET_DMA_STATUS_TI_GET(status)) {
        ...
        /* Call NetX deferred driver processing.  */
        p->nx_driver_information_deferred_events |= NX_DRIVER_DEFERRED_PACKET_TRANSMITTED;
        _nx_ip_driver_deferred_processing(p->nx_driver_information_ip_ptr);
#else
...
VOID isr_enet0(VOID)
{
    isr_enet(HPM_ENET0, enet0_drv_info_p);
}
#endif
...
VOID isr_enet1(VOID)
{
    isr_enet(HPM_ENET1, enet1_drv_info_p);
}
#endif

#ifndef NX_ENABLE_VLAN
static VOID _nx_driver_transfer_to_netx(NX_DRIVER_INFORMATION *p, NX_IP *ip_ptr, NX_PACKET *packet_ptr);
#endif

ATTR_RAMFUNC
NX_PACKET *_nx_driver_hardware_get_frame(NX_DRIVER_INFORMATION *p, enet_rx_desc_t **parent_rx_desc_list_cur, UINT rx_desc_count)
{
    enet_frame_t frame = {0};
    NX_PACKET *packet_ptr_rx = NULL;
    (void)parent_rx_desc_list_cur;
    (void)rx_desc_count;
    frame = enet_get_received_frame_interrupt(&p->desc.rx_desc_list_cur, &p->desc.rx_frame_info, ENET_RX_BUFF_COUNT);
    if (frame.length > 0) {
        if (nx_packet_allocate(p->nx_driver_information_packet_pool_ptr, &packet_ptr_rx,
                NX_RECEIVE_PACKET, NX_NO_WAIT) == NX_SUCCESS) {
            memcpy((VOID *)packet_ptr_rx->nx_packet_data_start, (VOID *)frame.buffer, frame.length);
        }
    }
    /* Release descriptors to DMA */
    enet_rx_desc_t *dma_rx_desc = p->desc.rx_frame_info.fs_rx_desc;

    /* Set Own bit in Rx descriptors: gives the buffers back to DMA */
    for (UINT i = 0; i < p->desc.rx_frame_info.seg_count; i++) {
        dma_rx_desc->rdes0_bm.own = 1;
        dma_rx_desc = (enet_rx_desc_t *)(dma_rx_desc->rdes3_bm.next_desc);
    }
    /* Clear Segment_Count */
    p->desc.rx_frame_info.seg_count = 0;
...
static UINT _nx_driver_hardware_initialize(NX_IP_DRIVER *driver_req_ptr)
{
    ...
    /* Setup indices.  */
    p->nx_driver_information_receive_current_index = 0;
    p->nx_driver_information_transmit_current_index = 0;
    p->nx_driver_information_transmit_release_index = 0;

    /* Clear the number of buffers in use counter.  */
    p->nx_driver_information_number_of_transmit_buffers_in_use = 0;

    /* Make sure there are receive packets... otherwise, return an error.  */
    if (p->nx_driver_information_packet_pool_ptr == NULL) {
    ...
    memset((UCHAR *)&p->desc.tx_buff_cfg, 0x00, sizeof(p->desc.tx_buff_cfg));
    memset((UCHAR *)&p->desc.rx_buff_cfg, 0x00, sizeof(p->desc.rx_buff_cfg));
    if (p->base == HPM_ENET0) {
        memset((UCHAR *)rx_buff_eth0, 0x00, sizeof(rx_buff_eth0));
    }
#ifdef HPM_ENET1
    else {
        memset((UCHAR *)rx_buff_eth1, 0x00, sizeof(rx_buff_eth1));
    }
#endif
#if (NETX_TX_DATA_COPY_ALGORITHM == NETX_DATA_COPY_CPU)
    if (p->base == HPM_ENET0) {
        memset((UCHAR *)tx_buff_eth0, 0x00, sizeof(tx_buff_eth0));
    }
#ifdef HPM_ENET1
    else {
        memset((UCHAR *)tx_buff_eth1, 0x00, sizeof(tx_buff_eth1));
    }
#endif
#endif
    if (p->base == HPM_ENET0) {
        p->desc.tx_desc_list_head = (enet_tx_desc_t *)core_local_mem_to_sys_address(BOARD_RUNNING_CORE, (UINT)dma_tx_desc_tab_eth0);
        p->desc.rx_desc_list_head = (enet_rx_desc_t *)core_local_mem_to_sys_address(BOARD_RUNNING_CORE, (UINT)dma_rx_desc_tab_eth0);
    }
#ifdef HPM_ENET1
    else {
        p->desc.tx_desc_list_head = (enet_tx_desc_t *)core_local_mem_to_sys_address(BOARD_RUNNING_CORE, (UINT)dma_tx_desc_tab_eth1);
        p->desc.rx_desc_list_head = (enet_rx_desc_t *)core_local_mem_to_sys_address(BOARD_RUNNING_CORE, (UINT)dma_rx_desc_tab_eth1);
    }
#endif

    if (p->base == HPM_ENET0) {
        p->desc.rx_buff_cfg.buffer = core_local_mem_to_sys_address(BOARD_RUNNING_CORE, (UINT)rx_buff_eth0);
#if (NETX_TX_DATA_COPY_ALGORITHM == NETX_DATA_COPY_CPU)
        p->desc.tx_buff_cfg.buffer = core_local_mem_to_sys_address(BOARD_RUNNING_CORE, (UINT)tx_buff_eth0);
#endif
    }
#ifdef HPM_ENET1
    else {
        p->desc.rx_buff_cfg.buffer = core_local_mem_to_sys_address(BOARD_RUNNING_CORE, (UINT)rx_buff_eth1);
#if (NETX_TX_DATA_COPY_ALGORITHM == NETX_DATA_COPY_CPU)
        p->desc.tx_buff_cfg.buffer = core_local_mem_to_sys_address(BOARD_RUNNING_CORE, (UINT)tx_buff_eth1);
#endif
    }
    p->desc.rx_buff_cfg.count = ENET_RX_BUFF_COUNT;
    p->desc.rx_buff_cfg.size = ENET_RX_BUFF_SIZE;
#endif
    p->desc.tx_buff_cfg.count = ENET_TX_BUFF_COUNT;
    p->desc.tx_buff_cfg.size = ENET_TX_BUFF_SIZE;

    /*Get a default control config for tx descriptor */
    enet_get_default_tx_control_config(p->base, &enet_tx_control_config);
    enet_tx_control_config.enable_ioc = true;
    /* Set the control config for tx descriptor */
    memcpy(&p->desc.tx_control_config, &enet_tx_control_config, sizeof(enet_tx_control_config_t));

    /* Get MAC address */
    if (p->base == HPM_ENET0) {
        if (_hardware_get_mac_address(mac_eth0, 0)) {
            return NX_DRIVER_ERROR;
        }
    } else {
        if (_hardware_get_mac_address(mac_eth1, 1)) {
            return NX_DRIVER_ERROR;
        }
    }
...
ATTR_RAMFUNC
static UINT _nx_driver_hardware_packet_send(NX_PACKET *packet_ptr, NX_IP_DRIVER *driver_req_ptr)
{
    ...
    enet_tx_desc_t *tx_desc_list_cur = p->desc.tx_desc_list_cur;
    ...
    for (pktIdx = packet_ptr; pktIdx != NULL; pktIdx = pktIdx->nx_packet_next) {
        ...
        for (i = 0; i < ENET_TX_BUFF_COUNT; i++) {
            enet_tx_desc_t *desc = p->desc.tx_desc_list_head + i;
            if (p->desc.tx_desc_list_cur == desc) {
                tx_des_index = i;
                break;
            }
        }
        ...
        p->nx_driver_information_transmit_packets[tx_des_index] = pktIdx;
        p->desc.tx_desc_list_cur->tdes2_bm.buffer1 = core_local_mem_to_sys_address(BOARD_RUNNING_CORE, (UINT)pktIdx->nx_packet_prepend_ptr);
    ...
    /* Disable interrupts.  */
    TX_DISABLE
    enet_prepare_tx_desc(p->base, &(p->desc.tx_desc_list_cur), &(p->desc.tx_control_config), frame_length, p->desc.tx_buff_cfg.size);
    TX_RESTORE
    return (NX_SUCCESS);
}
...
static VOID _nx_driver_hardware_packet_receive(NX_IP_DRIVER *driver_req_ptr)
{
    ...
    while (1) {
        enet_rx_desc_t **rx_desc_list_cur = &(p->desc.rx_desc_list_cur);

        /* Try to get the enet data from hardware */
        packet_ptr_rx = _nx_driver_hardware_get_frame(p, rx_desc_list_cur, ENET_RX_BUFF_COUNT);
        if (packet_ptr_rx == NULL) {
            break;
        }

#ifdef NX_ENABLE_VLAN
        nx_link_ethernet_packet_received(p->nx_driver_information_ip_ptr,
                                         p->nx_driver_information_interface->nx_interface_index,
                                         packet_ptr_rx, NX_NULL);
#else
        _nx_driver_transfer_to_netx(p, p->nx_driver_information_ip_ptr, packet_ptr_rx);
#endif
    }
}
...
static UINT _nx_driver_hardware_enable(NX_IP_DRIVER *driver_req_ptr)
{
    ...
    /* Set MAC0 address */
    if (p->base == HPM_ENET0) {
        enet_config.mac_addr_high[0] = mac_eth0[5] << 8 | mac_eth0[4];
        enet_config.mac_addr_low[0] = mac_eth0[3] << 24 | mac_eth0[2] << 16 | mac_eth0[1] << 8 | mac_eth0[0];
    }
#ifdef HPM_ENET1
    else {
        enet_config.mac_addr_high[0] = mac_eth1[5] << 8 | mac_eth1[4];
        enet_config.mac_addr_low[0] = mac_eth1[3] << 24 | mac_eth1[2] << 16 | mac_eth1[1] << 8 | mac_eth1[0];
    }
#endif
    enet_config.valid_max_count = 1;

    /* Set DMA PBL */
    enet_config.dma_pbl = board_get_enet_dma_pbl(p->base);

    /* Set SARC */
    enet_config.sarc = enet_sarc_replace_mac0;

    /* Enable Enet IRQ */
    board_enable_enet_irq(p->base);

    /* Get the default interrupt config */
    enet_get_default_interrupt_config(p->base, &int_config);
    ...
    enet_controller_netx_init(p->base, p->itf, &p->desc, &enet_config, &int_config);
...
static VOID _nx_driver_transfer_to_netx(NX_DRIVER_INFORMATION *p, NX_IP *ip_ptr, NX_PACKET *packet_ptr)
{
    ...
    packet_ptr->nx_packet_ip_interface = p->nx_driver_information_interface;
...
VOID nx_driver_set_interface_name(UINT idx, const CHAR *interface_name)
{
    assert(idx < 2);
    nx_hpmicro_interface_name[idx] = interface_name;
}
...
UINT _nx_driver_initialize(NX_IP_DRIVER *driver_req_ptr)
{
    ...
    p->nx_driver_information_state = NX_DRIVER_STATE_NOT_INITIALIZED;

    /* Setup the default packet pool for the driver's received packets.  */

    p->nx_driver_information_packet_pool_ptr = driver_req_ptr->nx_ip_driver_ptr->nx_ip_default_packet_pool;

    /* Clear the deferred events for the driver.  */
    p->nx_driver_information_deferred_events = 0;
    ...
    if (status == NX_SUCCESS) {
        ...
        /* Setup the physical address of this IP instance.  Increment the
           ...
           ethernet.  */
        if (p->base == HPM_ENET0) {
            interface_ptr->nx_interface_physical_address_msw = (ULONG)((mac_eth0[0] << 8) | (mac_eth0[1]));
            interface_ptr->nx_interface_physical_address_lsw = (ULONG)((mac_eth0[2] << 24) | (mac_eth0[3] << 16) | (mac_eth0[4] << 8) | (mac_eth0[5]));
        } else {
            interface_ptr->nx_interface_physical_address_msw = (ULONG)((mac_eth1[0] << 8) | (mac_eth1[1]));
            interface_ptr->nx_interface_physical_address_lsw = (ULONG)((mac_eth1[2] << 24) | (mac_eth1[3] << 16) | (mac_eth1[4] << 8) | (mac_eth1[5]));
        }
        ...
        p->nx_driver_information_state = NX_DRIVER_STATE_INITIALIZED;
...
static VOID _nx_driver_packet_send(NX_IP_DRIVER *driver_req_ptr)
{
    ...
    if (p->nx_driver_information_state != NX_DRIVER_STATE_LINK_ENABLED) {
    ...
    *(ethernet_frame_ptr + 3) = (driver_req_ptr->nx_ip_driver_interface->nx_interface_physical_address_lsw << 16) | ether_type;
    ...
    status = _nx_driver_hardware_packet_send(packet_ptr, driver_req_ptr);
...
static VOID _nx_driver_link_enable(NX_IP_DRIVER *driver_req_ptr)
{
    ...
    if (p->nx_driver_information_state < NX_DRIVER_STATE_INITIALIZED) {
    ...
    if (status == NX_SUCCESS) {
        ...
        p->nx_driver_information_state = NX_DRIVER_STATE_LINK_ENABLED;
...
static VOID _nx_driver_deferred_processing(NX_IP_DRIVER *driver_req_ptr)
{
    ...
    deferred_events = p->nx_driver_information_deferred_events;
    p->nx_driver_information_deferred_events = 0;
    ...
    if (deferred_events & NX_DRIVER_DEFERRED_PACKET_RECEIVED) {
        ...
        _nx_driver_hardware_packet_receive(driver_req_ptr);
    }
    ...
    if (deferred_events & NX_DRIVER_DEFERRED_PACKET_TRANSMITTED) {
        ...
        _nx_driver_hardware_packet_transmitted(driver_req_ptr);
    }
...
VOID _nx_driver_hpm(NX_IP_DRIVER *driver_req_ptr)
{
    ...
    switch (driver_req_ptr->nx_ip_driver_command) {
        case NX_LINK_INTERFACE_ATTACH: {
            p->nx_driver_information_interface = (NX_INTERFACE *)(driver_req_ptr->nx_ip_driver_interface);
            break;
        }
