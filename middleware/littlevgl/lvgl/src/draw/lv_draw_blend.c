    ...
    if(mask_res == LV_DRAW_MASK_RES_FULL_COVER) {
        if(opa > LV_OPA_MAX) {
            ...
            if((draw_area_w > 8) && (draw_area_h > 8)) {
                lv_gpu_hpm_pdma_fill(disp_buf_first, disp_w, color, draw_area_w, draw_area_h, opa);
                return;
            }
#endif
...
#elif LV_USE_GPU_HPM_PDMA
            if((draw_area_w > 8) && (draw_area_h > 8)) {
                lv_gpu_hpm_pdma_blit(disp_buf_first, disp_w, map_buf_first, map_w, draw_area_w, draw_area_h, opa);
                return;
            }
#endif
