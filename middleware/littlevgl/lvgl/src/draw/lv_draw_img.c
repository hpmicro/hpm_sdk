...
#elif LV_USE_GPU_HPM_PDMA
    #include "../gpu/lv_gpu_hpm_pdma.h"
#endif
                                              ...
                                              bool chroma_key, bool alpha_byte)
{
    ...
    /*Simple case without masking and transformations*/
        ...
        lv_gpu_nxp_pxp_disable_recolor();
            ...
            chroma_key == true && draw_dsc->recolor_opa == LV_OPA_TRANSP) { /*copy with color keying (+ alpha)*/
        lv_gpu_hpm_pdma_enable_color_key();
        _lv_blend_map(clip_area, map_area, (lv_color_t *)map_p, NULL, LV_DRAW_MASK_RES_FULL_COVER, draw_dsc->opa,
                      draw_dsc->blend_mode);
        lv_gpu_hpm_pdma_disable_color_key();
    }
    else if(other_mask_cnt == 0 && draw_dsc->angle == 0 && draw_dsc->zoom == LV_IMG_ZOOM_NONE && alpha_byte == false &&
            chroma_key == false && draw_dsc->recolor_opa != LV_OPA_TRANSP) { /*copy with recolor (+ alpha)*/
        lv_gpu_hpm_pdma_enable_recolor(draw_dsc->recolor, draw_dsc->recolor_opa);
        _lv_blend_map(clip_area, map_area, (lv_color_t *)map_p, NULL, LV_DRAW_MASK_RES_FULL_COVER, draw_dsc->opa,
                      draw_dsc->blend_mode);
        lv_gpu_hpm_pdma_disable_recolor();
    }
#endif
...
#elif LV_USE_GPU_HPM_PDMA && (LV_COLOR_DEPTH == 32)
            if((draw_area_w > 8) && (draw_area_h > 8)) {
                int32_t disp_w = lv_area_get_width(disp_area);
                lv_color_t * disp_buf = draw_buf->buf_act;
                lv_color_t * disp_buf_first = disp_buf + disp_w * draw_area.y1 + draw_area.x1;
                lv_gpu_hpm_pdma_blit(disp_buf_first, disp_w, (const lv_color_t *)map_buf_tmp, map_w, draw_area_w,
                                         draw_area_h, draw_dsc->opa);
                return;
            }
#endif
