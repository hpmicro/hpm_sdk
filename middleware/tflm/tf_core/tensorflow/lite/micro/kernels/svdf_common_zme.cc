    ...
    for (int b = 0; b < n_batch; b++) {
      ...
      for (int r = 0; r < n_filter; r++) {
        ...
        const int8_t* vector_in_batch = input + b * n_input;
         for (int c = 0; c < n_input; c += 16) {
           asm("vlbsext v12, %[src]"
               :
               : [src] "r"(&matrix_ptr[c + r * n_input])
               :);
           asm("vlbsext v16, %[src]" : : [src] "r"(&vector_in_batch[c]) :);
           asm("vsube32 v16, %0, v16" : : "r"(data.input_zero_point) :);
           asm("vdot %0, v12, v16" : "+r"(dot_prod)::);
        }
