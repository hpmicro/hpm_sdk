...
int DecodeCipQoSAttribute(void *const data,
	...
	const EipUint8 **const cip_message = (const EipUint8 **const )message_router_request->data;

	int number_of_decoded_bytes = -1;

	if (NULL != cip_message) {

		CipUsint attribute_value_received = GetUsintFromMessage((const CipOctet **const)&cip_message);
