    ...
    for (;;) {
        ...
        case DHCP_WAIT_ADDRESS: {
            ...
            if (IPaddress != 0) {
                ...
                sprintf((char *)iptxt, " %d.%d.%d.%d", (uint8_t)(gnetif.ip_addr.addr), (uint8_t)(gnetif.ip_addr.addr >> 8), \
				        (uint8_t)(gnetif.ip_addr.addr >> 16), (uint8_t)(gnetif.ip_addr.addr >> 24));
                printf("IP addr: %s\r\n", iptxt);

                sprintf((char *)iptxt, " %d.%d.%d.%d", (uint8_t)(gnetif.netmask.addr), (uint8_t)(gnetif.netmask.addr >> 8), \
				        (uint8_t)(gnetif.netmask.addr >> 16), (uint8_t)(gnetif.netmask.addr >> 24));
                printf("Netmask addr: %s\r\n", iptxt);

                sprintf((char *)iptxt, " %d.%d.%d.%d", (uint8_t)(gnetif.gw.addr), (uint8_t)(gnetif.gw.addr >> 8), \
				        (uint8_t)(gnetif.gw.addr >> 16), (uint8_t)(gnetif.gw.addr >> 24));
                printf("GW addr: %s\r\n", iptxt);

                sprintf((char *)mactxt, " %02x-%02x-%02x-%02x-%02x-%02x", gnetif.hwaddr[0], gnetif.hwaddr[1], gnetif.hwaddr[2], \
