...
#define MOTOR0_SPD                    (0.0) /*r/s   delta:0.1r/s    1-40r/s */
...
void motor_init(void)
{
    ...
    motor0.cfg.control.currentd_pid_cfg.cfg.kp =
        motor0.cfg.mcl.physical.motor.ls * (powf(MOTOR0_CURRENT_LOOP_BANDWIDTH * 2 * MCL_PI, 2)) * motor0.cfg.mcl.physical.time.current_loop_ts * 1.5f;
    motor0.cfg.control.currentd_pid_cfg.cfg.ki =
        motor0.cfg.mcl.physical.motor.res * (powf(MOTOR0_CURRENT_LOOP_BANDWIDTH * 2 * MCL_PI, 2)) * motor0.cfg.mcl.physical.time.current_loop_ts * 1.5f;
    ...
    motor0.cfg.control.currentq_pid_cfg.cfg.kp =
        motor0.cfg.mcl.physical.motor.ls * (powf(MOTOR0_CURRENT_LOOP_BANDWIDTH * 2 * MCL_PI, 2)) * motor0.cfg.mcl.physical.time.current_loop_ts * 1.5f;
    motor0.cfg.control.currentq_pid_cfg.cfg.ki =
        motor0.cfg.mcl.physical.motor.res * (powf(MOTOR0_CURRENT_LOOP_BANDWIDTH * 2 * MCL_PI, 2)) * motor0.cfg.mcl.physical.time.current_loop_ts * 1.5f;
    ...
    hpm_mcl_loop_init(&motor0.loop, &motor0.cfg.loop, &motor0.cfg.mcl, &motor0.encoder, &motor0.analog, &motor0.control, &motor0.drivers, NULL);
...
void motor0_speed_loop_para_init(void)
{
    ...
    motor0.cfg.control.currentd_pid_cfg.cfg.kp =
        motor0.cfg.mcl.physical.motor.ls * (powf(MOTOR0_CURRENT_LOOP_BANDWIDTH / 100 * 2 * MCL_PI, 2)) * motor0.cfg.mcl.physical.time.current_loop_ts * 1.5f;
    motor0.cfg.control.currentd_pid_cfg.cfg.ki =
        motor0.cfg.mcl.physical.motor.res * (powf(MOTOR0_CURRENT_LOOP_BANDWIDTH / 100 * 2 * MCL_PI, 2)) * motor0.cfg.mcl.physical.time.current_loop_ts * 1.5f;
    motor0.cfg.control.currentq_pid_cfg.cfg.kp =
        motor0.cfg.mcl.physical.motor.ls * (powf(MOTOR0_CURRENT_LOOP_BANDWIDTH / 100 * 2 * MCL_PI, 2)) * motor0.cfg.mcl.physical.time.current_loop_ts * 1.5f;
    motor0.cfg.control.currentq_pid_cfg.cfg.ki =
        motor0.cfg.mcl.physical.motor.res * (powf(MOTOR0_CURRENT_LOOP_BANDWIDTH / 100 * 2 * MCL_PI, 2)) * motor0.cfg.mcl.physical.time.current_loop_ts * 1.5f;
...
hpm_mcl_stat_t encoder_get_theta(float *theta)
{
    ...
    return hpm_mcl_abz_get_theta(BOARD_BLDC_QEI_BASE, BOARD_BLDC_QEI_FOC_PHASE_COUNT_PER_REV, -((MCL_PI * 2) / BOARD_BLDC_QEI_FOC_PHASE_COUNT_PER_REV), theta);
#endif
#ifdef HPMSOC_HAS_HPMSDK_QEIV2
    return hpm_mcl_abz_get_theta(BOARD_BLDC_QEIV2_BASE, BOARD_BLDC_QEI_FOC_PHASE_COUNT_PER_REV, -((MCL_PI * 2) / BOARD_BLDC_QEI_FOC_PHASE_COUNT_PER_REV),
                                 theta);
#endif
}
...
{
#ifdef HPMSOC_HAS_HPMSDK_QEI
    return hpm_mcl_abz_get_abs_theta(BOARD_BLDC_QEI_BASE, BOARD_BLDC_QEI_FOC_PHASE_COUNT_PER_REV, ((MCL_PI * 2) / BOARD_BLDC_QEI_FOC_PHASE_COUNT_PER_REV),
                                     -abs_position_theta, theta);
#endif
#ifdef HPMSOC_HAS_HPMSDK_QEIV2
    return hpm_mcl_abz_get_abs_theta(BOARD_BLDC_QEIV2_BASE, BOARD_BLDC_QEI_FOC_PHASE_COUNT_PER_REV, ((MCL_PI * 2) / BOARD_BLDC_QEI_FOC_PHASE_COUNT_PER_REV),
                                     -abs_position_theta, theta);
#endif
}
...
hpm_mcl_stat_t disable_all_pwm_output(void)
{
    ...
    pwm_set_force_output(MOTOR0_BLDCPWM,
                         PWM_FORCE_OUTPUT(BOARD_BLDC_UH_PWM_OUTPIN, pwm_output_0) | PWM_FORCE_OUTPUT(BOARD_BLDC_UL_PWM_OUTPIN, pwm_output_0) |
                             PWM_FORCE_OUTPUT(BOARD_BLDC_VH_PWM_OUTPIN, pwm_output_0) | PWM_FORCE_OUTPUT(BOARD_BLDC_VL_PWM_OUTPIN, pwm_output_0) |
                             PWM_FORCE_OUTPUT(BOARD_BLDC_WH_PWM_OUTPIN, pwm_output_0) | PWM_FORCE_OUTPUT(BOARD_BLDC_WL_PWM_OUTPIN, pwm_output_0));
...
hpm_mcl_stat_t qei_init(void)
{
    ...
    qei_phase_config(BOARD_BLDC_QEI_BASE, BOARD_BLDC_QEI_FOC_PHASE_COUNT_PER_REV, qei_z_count_inc_on_phase_count_max, true);
    qei_phase_cmp_set(BOARD_BLDC_QEI_BASE, 4, false, qei_rotation_dir_cmp_ignore);
    qei_load_read_trigger_event_enable(BOARD_BLDC_QEI_BASE, QEI_EVENT_POSITIVE_COMPARE_FLAG_MASK);
...
static void timer_init(void)
{
    ...
    clock_add_to_group(BOARD_BLDC_TMR_CLOCK, BOARD_RUNNING_CORE & 0x1);
...
{
    /* pos data from qei */
    trgm_pos_matrix_config(HPM_TRGM0, BOARD_BLDC_TRGM_POS_MATRIX_TO_VSC, BOARD_BLDC_TRGM_POS_MATRIX_FROM_QEI,
                           true); /* position invert: motor foreward rotation, position decrease */
...
void setup_moter(void)
{
    ...
    printf("\r\nSpeed mode, motor run, speed is: %f.\r\n", (double)g_motor_speed);
}

float get_motor_current_speed(void)
{
    return hpm_mcl_encoder_get_speed(motor0.loop.encoder) / (2 * MCL_PI);
}
