...
ULONG thread_https_stack[DEMO_STACK_SIZE >> 2];
...
static UINT test_device_cert_der_len = 982;
...
static UINT test_ca_cert_der_len = 971;
...
static CHAR ram_disk_memory[1024 * 64];
...
VOID tx_application_define(VOID *first_unused_memory)
{
    ...
    status = nx_ip_create(&server_ip, "HTTP Server IP", SAMPLE_IPV4_ADDRESS,
        SAMPLE_IPV4_MASK, &server_pool, _nx_driver_hpm,
        (VOID *)ip_stack, sizeof(ip_stack), 1);
    if (status)
        error_counter++;

    /* Enable ARP and supply ARP cache memory for the server IP instance.  */
    status = nx_arp_enable(&server_ip, (VOID *)arp_area, sizeof(arp_area));
    if (status)
        error_counter++;
