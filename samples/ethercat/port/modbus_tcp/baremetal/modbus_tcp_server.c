...
uint8_t tcp_s_rx_buf[MODBUS_TCP_BUFF_SIZE];
...
hpm_stat_t network_tcp_server_init(void)
{
    ...
    tcp_bind(pcb, IP_ADDR_ANY, MODBUS_TCP_SERVER_PORT);
    pcb = tcp_listen(pcb);
...
int network_tcp_s_receive(uint8_t *data, int len)
{
    ...
    if (network_rx.front_index > network_rx.rear_index) {
        rx_data_size = (MODBUS_TCP_BUFF_SIZE - network_rx.front_index) + network_rx.rear_index;
    } else {
    ...
    for (int i = 0; i < rx_data_size; i++) {
        if ((network_rx.front_index + i) < MODBUS_TCP_BUFF_SIZE) {
            data[i] = tcp_s_rx_buf[network_rx.front_index + i];
        } else {
            data[i] = tcp_s_rx_buf[network_rx.front_index + i - MODBUS_TCP_BUFF_SIZE];
        }
    }
...
static err_t tcp_s_recv(void *arg, struct tcp_pcb *tpcb, struct pbuf *p, err_t err)
{
    ...
    if (p != NULL) {
        ...
        client_pcb = tpcb;
        remain_len = MODBUS_TCP_BUFF_SIZE - network_rx.rear_index;
        ...
        network_rx.rear_index %= MODBUS_TCP_BUFF_SIZE;
