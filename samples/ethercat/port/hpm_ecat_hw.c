...
#include "sys_arch.h" /* lwip sys_arch.h */
#endif
...
#if defined(ECAT_SSC_TIMER_USE_RTOS_TICK) && ECAT_SSC_TIMER_USE_RTOS_TICK
#if defined(__ENABLE_FREERTOS) && __ENABLE_FREERTOS
static TickType_t time_base = 0;
void ecat_time_init_rtos(void);
#endif /* __ENABLE_FREERTOS */
#else
...
hpm_stat_t ecat_hardware_init(ESC_Type *esc_ptr)
{
    ...
    /* disable sync0/sync1 map to PDI irq */
    esc_enable_sync_irq_to_pdi_irq(esc_ptr, false, false);
#endif
...
#if defined(ESC_SUPPORT_ECAT_LED) && ESC_SUPPORT_ECAT_LED
    ...
    if (status_success == stat) {
        ...
        esc_enable_irq(esc_ptr, esc_irq_mask_all); /* Enable IRQ in ESC register */
        ENABLE_ESC_INT(); /* EtherCAT interrupts in PLIC */
    } else {
        /* Close operation */
    }
...
void ecat_time_init_rtos(void)
{
    time_base = xTaskGetTickCount();
}

UINT32 HW_GetTimer(void)
{
    TickType_t current_ticks, elapsed_ticks;
    uint32_t elapsed_ms;

    current_ticks = xTaskGetTickCount();
    elapsed_ticks = current_ticks - time_base;
    elapsed_ms = elapsed_ticks * portTICK_PERIOD_MS;  /* calculate time based on freertos tick */
...
{
    time_base = xTaskGetTickCount();  /* update time base */
}

#if defined(ECAT_TIMER_INT) && ECAT_TIMER_INT
/* freertos Tick Hook（1ms, configUSE_TICK_HOOK should be defined 1 in FreeRTOSConfig.h */
...
void ecat_time_ms_isr(void)
{
    if (gptmr_check_status(ECAT_TIMER_GPTMR, GPTMR_CH_RLD_STAT_MASK(ECAT_TIMER_GPTMR_CH))) {
        gptmr_clear_status(ECAT_TIMER_GPTMR, GPTMR_CH_RLD_STAT_MASK(ECAT_TIMER_GPTMR_CH));
...
void ecat_sync0_isr(void)
{
    ...
    volatile UINT8 SyncState = 0;
    HW_EscReadByte(SyncState, 0x098E); /* Readbyte, acknowledge Sync0 */
#else
    volatile UINT32 SyncState = 0;
    HW_EscReadDWord(SyncState, ESC_DC_SYNC_STATUS); /* ReadDWord, acknowledge Sync0 and Sync1 */
#endif
    (void) SyncState;
...
#if defined(HPM_IP_FEATURE_ESC_BYTE_READ) && HPM_IP_FEATURE_ESC_BYTE_READ
    volatile UINT8 SyncState = 0;
    HW_EscReadByte(SyncState, 0x098F); /* Readbyte, acknowledge Sync0 */
#else
    volatile UINT32 SyncState = 0;
    HW_EscReadDWord(SyncState, ESC_DC_SYNC_STATUS); /* ReadDWord, acknowledge Sync0 and Sync1 */
#endif
...
{
    #warning "Please implement the function for MCU to control LED."
}
#endif
...
#if !defined(ECAT_SSC_TIMER_USE_RTOS_TICK) || !ECAT_SSC_TIMER_USE_RTOS_TICK
    intc_m_disable_irq(ECAT_TIMER_GPTRM_IRQ);
#endif /* ECAT_SSC_TIMER_USE_RTOS_TICK */
}
