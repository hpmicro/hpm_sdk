...
The ECAT_IO example demonstrates the steps to implement an ECAT digital IO slave station with the ESC peripheral and the slave stack code (SSC).

This routine program supports initializing the EEPROM data(without limiting the use of Flash simulation EEPROM or actual EEPROM) of the ESC, simplifying the process of updating the ESC's EEPROM.
...
~~~~~~~~~~~~~~~~~~~~~~~
  ...
  - Set the "Hardware" attribute "ESC_EEPROM_EMULATION = 0" in SSC Tool when generating code, then regenerate the slave protocol stack code.

  - Comment out the "#define SET_EEPROM_PTR pEEPROM = aEepromData;" line in the generated protocol stack code ecatappl.c to resolve compilation errors.

  - In the file `CMakeLists.txt`, set "set(CONFIG_EEPROM_EMULATION 0)" and rebuild the project.
...
~~~~~~~~~~~~~~~~~~~
  ...
  **Note**: On HPM5E00, hardware supports setting sync0 and sync1 to not trigger PDI interrupts, so there is no need to use this patch again.
