...
#define HYBRID_XPI_NOR_BASE_ADDR (0xB0000000UL)
...
#if !defined(HPM_XPI_SOC_SUPPORT_HYBRID_MODE) || (HPM_XPI_SOC_SUPPORT_HYBRID_MODE == 0)
    printf("This example is not supported!\n");
    return 0;
#endif
...
static hpm_stat_t xpi_nor_test_last_sector(void)
{
    ...
    uint32_t erase_start = HYBRID_XPI_NOR_BASE_ADDR + (flash_size - sector_size);
...
static hpm_stat_t xpi_nor_stress_test(void)
{
    ...
    do {
        ...
        for (uint32_t j = 0; j < flash_size;) {
            uint32_t erase_addr = HYBRID_XPI_NOR_BASE_ADDR + j;
            status = rom_xpi_nor_erase_sector(base, chn, &s_xpi_nor_config, erase_addr);
            if (status != status_success) {
                ++err_cnt;
                break;
            }
        ...
        for (uint32_t j = 0; j < flash_size;) {
            uint32_t erase_addr = HYBRID_XPI_NOR_BASE_ADDR + j;
            status = rom_xpi_nor_erase_block(base, chn, &s_xpi_nor_config, erase_addr);
            if (status != status_success) {
                ++err_cnt;
                break;
            }
        ...
        for (uint32_t j = 0; j < flash_size;) {
            uint32_t program_addr = HYBRID_XPI_NOR_BASE_ADDR + j;
            status = rom_xpi_nor_program(base, chn, &s_xpi_nor_config, s_write_buf, program_addr, sizeof(s_write_buf));
            if (status != status_success) {
                ++err_cnt;
                break;
            }
        ...
        for (uint32_t j = 0; j < flash_size;) {
            uint32_t read_addr = HYBRID_XPI_NOR_BASE_ADDR + j;
            status = rom_xpi_nor_read(base, chn, &s_xpi_nor_config, s_read_buf, read_addr, sizeof(s_read_buf));
            if (status != status_success) {
                ++err_cnt;
                break;
            }
