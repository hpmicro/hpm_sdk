...
int32_t tpt_nn_conv_dw_HWC_s8_s8_s8_sft_bias_any(const q7_t * in_tensor,
                                  ...
                                  q7_t * tmp_buf)
{
    ...
    for (i_out_y = 0; i_out_y < out_tensor_dim_y; i_out_y++)
    {
        for (i_out_x = 0; i_out_x < out_tensor_dim_x; i_out_x++)
        {
            ...
            {
                // for each output
                int       conv_out = ((q31_t)(bias[i_ch_out]) << bias_lshift) + NN_ROUND(out_rshift);
                ...
                out_tensor[(i_out_y * out_tensor_dim_x + i_out_x) * out_tensor_ch + i_ch_out] =
                    (q7_t) NDS_ISA_SATS((conv_out >> out_rshift), 8);
            }
        }
    }
