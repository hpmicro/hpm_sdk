...
int32_t tpt_nn_conv_HWC_s16_s16_s16_sft_bias_fast(const q15_t * in_tensor,
    ...
    for (i = 0; i < out_tensor_ch; i++)
    {
        for (j = 0; j < out_tensor_dim; j++)
        {
            for (k = 0; k < out_tensor_dim; k++)
            {
                ...
                out_tensor[i + (j * out_tensor_dim + k) * out_tensor_ch] = (q15_t) NDS_ISA_SATS((conv_out >> out_rshift), 16);
            }
        }
    }
