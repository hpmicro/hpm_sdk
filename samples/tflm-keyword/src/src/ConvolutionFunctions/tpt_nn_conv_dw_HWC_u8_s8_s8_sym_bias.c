...
int32_t tpt_nn_conv_dw_HWC_u8_s8_s8_sym_bias(const u8_t * in_tensor,
    ...
    for (i_out_y = 0; i_out_y < out_tensor_dim; i_out_y++)
    {
        for (i_out_x = 0; i_out_x < out_tensor_dim; i_out_x++)
        {
            for (i_ch_out = 0; i_ch_out < out_tensor_ch; i_ch_out++)
            {
                ...
                out_tensor[(i_out_y * out_tensor_dim + i_out_x) * out_tensor_ch + i_ch_out] = NDS_ISA_SATS((conv_out >> post_rshift), 8);
            }
        }
    }
    return 0;
