...
int32_t tpt_nn_conv_HWC_s8_s8_s8_sft_bias_any(const q7_t * in_tensor,
                        ...
                        if (in_row >= 0 && in_col >= 0 && in_row < in_tensor_dim_y && in_col < in_tensor_dim_x)
                        {
                            for (l = 0; l < in_tensor_ch; l++)
                            {
                                conv_out +=
                                    ...
                                    ker_weight[i * in_tensor_ch * ker_dim_y * ker_dim_x +
                                               (m * ker_dim_x + n) * in_tensor_ch + l];
                            }
                        }
                    }
                }
                out_tensor[i + (j * out_tensor_dim_x + k) * out_tensor_ch] = (q7_t) NDS_ISA_SATS((conv_out >> out_rshift), 8);
            }
        }
    }
