...
int32_t tpt_nn_fc_u8_u8_s8_sym_bias_fast(const u8_t * in_vec,
                                    const q7_t * wt_mat,
                                    const uint16_t size,
                                    ...
                                    q15_t * in_tmp_buf)
{
    ...
    while (rowCnt)
    {
        ...
        else
        {
            ...
            *pO++ = NDS_ISA_SAT((sum >> post_rshift), 8);
        }
        if (sum2 < 0)
            *pO++ = 0;
        else
        {
            ...
            *pO++ = NDS_ISA_SAT((sum2 >> post_rshift), 8);
        }
        if (sum3 < 0)
            *pO++ = 0;
        else
        {
            ...
            *pO++ = NDS_ISA_SAT((sum3 >> post_rshift), 8);
        }
        if (sum4 < 0)
            *pO++ = 0;
        else
        {
            ...
            *pO++ = NDS_ISA_SAT((sum4 >> post_rshift), 8);
        }
    ...
    while (rowCnt)
    {
        ...
        else
        {
            ...
            *pO++ = NDS_ISA_SAT((ip_out >> post_rshift), 8);
        }
