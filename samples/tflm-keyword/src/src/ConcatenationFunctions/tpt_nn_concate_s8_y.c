...
void tpt_nn_concate_s8_y(const int8_t *in_tensor,
...
{
    const uint32_t num_iterations  = in_tensor_z * in_tensor_w;
    const uint32_t input_copy_size = in_tensor_x * in_tensor_y;
    const uint32_t output_stride   = in_tensor_x * out_tensor_y;

    out_tensor += out_offset_y * in_tensor_x;
    uint32_t i;
