...
static const uint8_t other_speed_config_descriptor[] = {
    USB_OTHER_SPEED_CONFIG_DESCRIPTOR_INIT(USB_HID_CONFIG_DESC_SIZ, 0x01, 0x01, USB_CONFIG_REMOTE_WAKEUP | USB_CONFIG_SELF_POWERED, USBD_MAX_POWER),
...
static void usbd_event_handler(uint8_t busid, uint8_t event)
{
    ...
    case USBD_EVENT_CONNECTED:
        printf("USBD_EVENT_CONNECTED\r\n");
        break;
    case USBD_EVENT_DISCONNECTED:
        printf("USBD_EVENT_DISCONNECTED\r\n");
        break;
    case USBD_EVENT_RESUME:
        printf("USBD_EVENT_RESUME\r\n");
        break;
    case USBD_EVENT_SUSPEND:
        printf("USBD_EVENT_SUSPEND\r\n");
        break;
    case USBD_EVENT_CONFIGURED:
        printf("USBD_EVENT_CONFIGURED\r\n");
        break;
...
void hid_mouse_test(uint8_t busid)
{
    ...
    if (gpio_read_pin(BOARD_APP_GPIO_CTRL, BOARD_APP_GPIO_INDEX, BOARD_APP_GPIO_PIN) == BOARD_BUTTON_PRESSED_VALUE) {
        if (usb_device_is_suspend(busid)) {
            /*!< send cmd to wakeup host */
            ...
            ret = usbd_ep_start_write(busid, HID_INT_EP, (uint8_t *)&mouse_cfg, 4);
            if (ret < 0) {
                return;
            }
        }
