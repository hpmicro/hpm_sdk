...
typedef struct motor0_par {
    ...
    hpm_hfi_para_t     inject_para;
    hpm_hfi_pll_para_t inject_pll;
    hpm_hfi_pole_detect_para_t inject_pole_detect;
...
MOTOR0_PARA motor0 = {  BLDC_CONTROL_FOC_PARA_DEFAULTS,
                        ...
                        BLDC_CONTROL_INJECT_PARA_DEFAULTS,
                        BLDC_CONTROL_INJECT_PLL_PARA_DEFAULTS,
                        BLDC_CONTROL_INJECT_POLE_DETECT_PARA_DEFAULTS,
                        NULL
};
...
void bldc_init_par(void)
{
    ...
    par->motorpar.func_smc_const = NULL;
    ...
    par->speedcalpar.i_speedacq = 40;
    par->speedcalpar.i_speedfilter = HPM_MOTOR_MATH_FL_MDF(0.02);
    par->speedcalpar.i_speedlooptime_s = HPM_MOTOR_MATH_FL_MDF(0.00005*40);
    ...
    par->currentqpipar.i_max = HPM_MOTOR_MATH_FL_MDF(PWM_RELOAD * 0.9);
    ...
    par->func_dqsvpwm =  hpm_mcl_hfi_loop;

    motor0.speedloop_para.func_pid  = hpm_mcl_bldc_foc_pi_contrl;
    motor0.speedloop_para.i_kp      = HPM_MOTOR_MATH_FL_MDF(40);
    ...
    motor0.inject_para.last_alpha = 0;
    ...
    motor0.inject_pll.mem_min = -3800;
    motor0.inject_pll.loop_s = 0.00005;
    motor0.inject_pll.low_pass = 0;
    ...
    motor0.inject_pole_detect.func = &hpm_mcl_hfi_pole_detect;
}
...
void isr_gptmr(void)
{
    ...
    if (gptmr_check_status(BOARD_BLDC_TMR_1MS, GPTMR_CH_CMP_IRQ_MASK(BOARD_BLDC_TMR_CH, BOARD_BLDC_TMR_CMP))) {
        ...
        if (timer_1ms >= 10) {
            ...
            if (motor0.inject_pole_detect.func(&motor0.foc_para, &motor0.inject_para, &motor0.inject_pll, &motor0.inject_pole_detect)) {
...
void motor0_current_loop(void)
{
    ...
    motor0.foc_para.electric_angle = motor0.inject_pll.theta;
    motor0.foc_para.samplcurpar.func_sampl(&motor0.foc_para.samplcurpar);
    motor0.foc_para.func_dqsvpwm(&motor0.foc_para, &motor0.inject_para, &motor0.inject_pll, &motor0.inject_pole_detect);
    motor0.foc_para.pwmpar.pwmout.func_set_pwm(&motor0.foc_para.pwmpar.pwmout);
#if SEGGER_RTT_ENABLE
    rtt_disp_val.theta_pll = motor0.inject_pll.theta * 180 / PI;
    rtt_disp_val.theta_swi = motor0.inject_pll.err_last;
#endif
}
...
{
    motor0_current_loop();
    motor0.foc_para.speedcalpar.speedtheta = motor0.inject_pll.theta * 180 / PI;
...
void adc_init(void)
{
    ...
    cfg.config.adc12.adc_clk_div    = adc12_clock_divider_4;
    ...
    ch_cfg.config.adc12_ch.sample_cycle  = 20;
...
#if SEGGER_RTT_ENABLE
    JS_RTT_Channel = 1;
    SEGGER_RTT_ConfigUpBuffer(JS_RTT_Channel, "JScope_F4F4F4", &JS_RTT_UpBuffer[0], sizeof(JS_RTT_UpBuffer), SEGGER_RTT_MODE_BLOCK_IF_FIFO_FULL);
#endif
    set_adcval_middle();
