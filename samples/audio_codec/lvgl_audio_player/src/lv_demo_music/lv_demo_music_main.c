...
/*    lv_obj_fade_in(icon_box, 1000, INTRO_TIME + 1000);*/
    ...
    LV_IMAGE_DECLARE(hpmicro_logo_large);
    lv_obj_t * logo = lv_image_create(lv_screen_active());
    lv_image_set_src(logo, &hpmicro_logo_large);
...
void _lv_demo_music_play(uint32_t id)
{
    if (!first_startted) {
        ...
        if (id != track_id) {
            _lv_demo_music_list_button_img(track_id, false);
            track_load(id);
...
void _lv_demo_music_resume(void)
{
    ...
    _lv_demo_music_list_button_img(track_id, true);
}
...
static lv_obj_t * create_title_box(lv_obj_t * parent)
{
    ...
    lv_obj_t *logo1;
    LV_IMAGE_DECLARE(hpmicro_logo);
    logo1 = lv_image_create(cont);
    lv_image_set_src(logo1, &hpmicro_logo);
