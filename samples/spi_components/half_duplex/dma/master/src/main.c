...
int main(void)
{
    ...
    uint32_t index = 0;
    board_init();
    printf("half_duplex spi master use dma\n");
    ...
    while (remain_size > 0) {
        ...
        if (hpm_spi_transmit_nonblocking(TEST_SPI, (uint8_t *)&sent_buff[index], transfer_size) != status_success) {
    ...
    while (remain_size > 0) {
        transfer_size = (remain_size > TRANSFER_COUNT_MAX) ? TRANSFER_COUNT_MAX : remain_size;
        if (hpm_spi_receive_nonblocking(TEST_SPI, (uint8_t *)&receive_buff[index], transfer_size) != status_success) {
