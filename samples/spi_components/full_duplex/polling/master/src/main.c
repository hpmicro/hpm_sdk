...
#define TEST_SPI_SCLK_FREQ          (10000000UL)
...
int main(void)
{
    ...
    uint32_t index = 0;
    board_init();
    printf("full_duplex spi master use polling\n");
    ...
    while (remain_size > 0) {
        ...
        if (hpm_spi_transmit_receive_blocking(TEST_SPI, (uint8_t *)&sent_buff[index], (uint8_t *)&receive_buff[index], transfer_size, 0xFFFFFFFF) != status_success) {
