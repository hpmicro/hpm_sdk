...
int main(void)
{
    ...
    board_init();
    printf("full_duplex spi slave use polling\n");
    ...
    /* step.2 full duplex, only support spi_single_io_mode */
    prepare_transfer_data();
    if (hpm_spi_transmit_receive_blocking(TEST_SPI, (uint8_t *)sent_buff, (uint8_t *)receive_buff, sizeof(sent_buff), 0xFFFFFFFF) != status_success) {
