...
static void server_task(void *param)
{
    ...
    /* process message */
    erpc_status_t status = erpc_server_run(erpc_server);
...
int main(void)
{
    ...
    if (xTaskCreate(server_task, "APP_TASK", configMINIMAL_STACK_SIZE + 256U, NULL, tskIDLE_PRIORITY + 1U, NULL) != pdPASS) {
