...
void opener_init(struct netif *netif)
{
    ...
    if ((g_end_stack == 0) && (eip_status == kEipStatusOk)) {
        osThreadDef(opener_thread, OPENER_THREAD_PRIO, 0, OPENER_STACK_SIZE);
        osThreadCreate(osThread(opener_thread), netif);
