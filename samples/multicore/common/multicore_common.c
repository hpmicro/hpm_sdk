...
void multicore_release_cpu(uint8_t cpu_id, uint32_t start_addr)
{
    ...
    uint32_t sec_core_img_sys_addr = core_local_mem_to_sys_address(cpu_id, (uint32_t)start_addr);
    ...
    if (!sysctl_is_cpu_released(HPM_SYSCTL, cpu_id)) {
        ...
        rom_sdp_memcpy(p_sdp_ctx, (void *)sec_core_img_sys_addr, sec_core_img, sec_core_img_size);
