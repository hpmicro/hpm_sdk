...
int main(void)
{
    ...
    sei_set_instr(BOARD_SEI, instr_idx++, SEI_INSTR_OP_RECV_WDG, 0, SEI_DAT_3, SEI_DAT_1, 8);  /* recv cmd */
    ...
    sei_set_instr(BOARD_SEI, instr_idx++, SEI_INSTR_OP_SEND, 0, SEI_DAT_5, SEI_DAT_1, 8);      /* send cmd */
...
void isr_sei(void)
{
    ...
    if (sei_get_irq_status(BOARD_SEI, BOARD_SEI_CTRL, sei_irq_latch0_event)) {
        ...
        printf("ADDR:%#x, CMD:%#x, POS:%#x, sample_tm1:%u, sample_tm2:%u, sample_interval:%d us\n",
