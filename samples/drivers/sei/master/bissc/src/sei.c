...
void isr_sei(void)
{
    ...
    if ((irq_flag & sei_irq_latch2_event) != 0) {
        ...
        rxd_fall_tm = sei_get_latch_time(BOARD_SEI, BOARD_SEI_CTRL, SEI_LATCH_2);
        delta = (rxd_fall_tm > sample_latch_tm) ? (rxd_fall_tm - sample_latch_tm) : (rxd_fall_tm - sample_latch_tm + 0xFFFFFFFFu);
        ...
        if (!s_rx_point_calc_flag) {
            ...
            if (s_rx_point_calc_tm > 2) {
                ...
                fall_pos = delta % baud_div;
                if ((fall_pos > (baud_div >> 2)) && (fall_pos < ((baud_div >> 2) * 3))) {
