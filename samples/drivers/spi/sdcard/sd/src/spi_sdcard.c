...
static void display_sdcard_info(void)
{
    if (sdcard_spi_get_card_info(&g_sd_info) == status_success) {
        ...
        printf("device block Size: %d block\n", (uint32_t)g_sd_info.csd.device_size);
...
static void test_write_read_last_block(void)
{
    ...
    uint32_t start_sector_addr = g_sd_info.csd.device_size - WRITE_READ_BLOCK_COUNT;
    uint32_t end_sector_addr   = g_sd_info.csd.device_size - 1U;
