...
- For SOCs that support interrupt triggering by detecting the rising edge/falling edge of the chip select CS signal,

when the rising edge/falling edge of the chip select signal is detected, the log would be seen on the terminal like:
