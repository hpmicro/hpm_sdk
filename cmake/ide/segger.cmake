...
function (generate_ses_project)
    ...
    string(FIND ${SES_TOOLCHAIN_VARIANT} "Terapines" exist)
    if (NOT ${exist} EQUAL -1)
        set(SES_USE_TOOLCHAIN_TPT 1)
        get_property(target_zcc_cflags TARGET ${HPM_SDK_ZCC_LIB_ITF} PROPERTY INTERFACE_COMPILE_OPTIONS)
    endif()
    ...
    elseif (SES_USE_TOOLCHAIN_TPT)
        # if Terapines toolchain has not been specified, cflags needs to be preserved
        foreach(f IN ITEMS ${target_gcc_cflags})
            string(FIND ${f} "-mabi=" exist)
            if(NOT ${exist} EQUAL -1)
                string(SUBSTRING ${f} 6 -1 SES_COMPILER_ABI)
        ...
        endforeach()
        if(target_zcc_cflags)
            foreach(f IN ITEMS ${target_zcc_cflags})
                if("${target_cflags}" STREQUAL "")
                    set(target_cflags ${f})
                else()
                    set(target_cflags "${target_cflags};${f}")
    ...
    elseif(SES_USE_TOOLCHAIN_TPT)
        # if Terapines toolchain has not been specified, gcc linker script should be used
        set(target_linker ${target_linker_script})
    else()
